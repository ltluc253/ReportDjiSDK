...
public class FetchMediaView extends BaseThreeBtnView {
        ...
        if (DJIModuleVerificationUtil.isCameraModuleAvailable() && mMedia != null) {
            ...
            mMedia.fetchMediaData(destDir, "testName", new DJIMediaManager.CameraDownloadListener<String>(){
                ...
                public void onProgress(long l, long l1) {
                    ...
                    str = "Downloading file 1 progress: " + (l1 * 100 / l);
                    message.obj = str;
                ...
                @Override
                public void onFailure(DJIError djiError) {
                    Utils.setResultToToast(getContext(), "Fetch media fail");
                }
            });
        }*/
        fetchMediaData(0);
    ...
    private void fetchMediaData(final int index) {
        mActionHandler.postDelayed(new Runnable() {
            @Override
            public void run() {
                if (DJIModuleVerificationUtil.isCameraModuleAvailable() && mMedias != null && index < mMedias.size() && index < 20) {
                    File destDir = new File(Environment.getExternalStorageDirectory().
                            getPath() + "/Dji_Sdk_Test/");
                    mMedias.get(index).fetchMediaData(destDir, mMedias.get(index).getFileNameWithoutExtension(), new DJIMediaManager.CameraDownloadListener<String>(){
                        String str;
                        @Override
                        public void onStart() {
                        ...
                        public void onProgress(long l, long l1) {
                            Message message = Message.obtain();
                            str = "Downloading file " + index + "/" + mMedias.size() + " progress: " + (l1 * 100 / l) + " - success: " + successCounter + ", fail: " + failCounter;
                            message.obj = str;
                            messageHandler.sendMessage(message);
                        }
